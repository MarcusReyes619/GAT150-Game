...
namespace kiko
{
	bool Json::Load(const std::string& filename, rapidjson::Document& document)
	{
		...
		/*if (!<read the file into the buffer using readFile() from FileIO.h, log warning if failed>)
		{
	...
	bool Json::Read(const rapidjson::Value& value, const std::string& name, int& data, bool required)
	{
		...
		//data = value[<get c string of name>].<get int function>;
	...
	bool Json::Read(const rapidjson::Value& value, const std::string& name, std::string& data, bool require)
	{
	...
	bool Json::Read(const rapidjson::Value& value, const std::string& name, vec2& data, bool require)
	{
	...
	bool Json::Read(const rapidjson::Value& value, const std::string& name, Color& data, bool require){
	...
	bool Json::Read(const rapidjson::Value& value, const std::string& name, Rect& data, bool require) {
	...
	bool Json::Read(const rapidjson::Value& value, const std::string& name, std::vector<std::string>& data, bool require) {
	...
	bool Json::Read(const rapidjson::Value& value, const std::string& name, std::vector<int>& data, bool require){
		// check if 'name' member exists and is an array
		if (!value.HasMember(name.c_str()) || !value[name.c_str()].IsArray())
		{
			if (require) ERROR_LOG("Cannot read required json data: " << name.c_str());
