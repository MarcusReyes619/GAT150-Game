...
#define INSTIANTIATE(classbase, classname) kiko::Factory::Instance().Create<kiko::classbase>(classname);
...
namespace kiko{
	...
	template <typename T>
	class PrototpyeCreator : public CreatorBase {

	public:



		PrototpyeCreator(std::unique_ptr<T> prototype): m_prototype{std::move(prototype)}{}
	...
	inline void Factory::RegisterPrototype(const std::string& key, std::unique_ptr<T> prototype)
	{
		...
		m_registry[key] = std::make_unique<PrototpyeCreator<T>>(std::move(prototype));
